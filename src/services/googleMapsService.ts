...
export class GoogleMapsService {
  ...
  async loadGoogleMaps(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      // Updated to use the new loading approach with places library
      script.src = `https://maps.googleapis.com/maps/api/js?key=${this.apiKey}&loading=async&libraries=places&callback=initGoogleMaps`;
      script.async = true;
      script.defer = true;
      ...
      (window as any).initGoogleMaps = () => {
        ...
        if (!servicesCheck.maps || !servicesCheck.places || !servicesCheck.directionsService || !servicesCheck.placesService) {
  ...
  async searchPlaces(query: string): Promise<PlaceResult[]> {
    ...
    try {
      ...
      // Use the legacy PlacesService for better compatibility
      ...
      return new Promise((resolve, reject) => {
        ...
        placesService.findPlaceFromQuery(request, (results, status) => {
          if (status === google.maps.places.PlacesServiceStatus.OK && results) {
            ...
            console.log('No places found for query:', query, 'Status:', status);
            resolve([]);
          }
        });
      });
